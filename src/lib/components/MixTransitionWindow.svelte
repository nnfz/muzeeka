<script lang="ts">
  ...
  const MAX_DETAIL_VIEW_SECS = 40;
  /** Never decode more than this many seconds for a detail window. */
  const MAX_DETAIL_RANGE_SECS = 55;
  const OVERVIEW_BINS_CAP = 1600;
  const DETAIL_BINS_CAP = 4096;
  const LOD_PAD_RATIO = 0.35;
  ...
  /** Prefer detail when it covers the view; otherwise stable overview. */
  function pickPeakBuf(
    lane: LaneState,
    viewStart: number,
    viewEnd: number,
    widthPx: number,
    ...
    if (
      ...
      densityOk(lane.detail, viewSpan, widthPx)
    ) {
      return lane.detail;
    }
   ...
   * Previous → end in view; next → start in view.
   */
  ...
  /**
   * Only when zoomed in tightly: fetch a short high-res window.
   * Zooming out reuses the cached overview — never re-decodes the whole file.
   */
  function ensureDetail(id: LaneId) {
    ...
    const pad = viewDur * LOD_PAD_RATIO;
  ...
  function scheduleLodRefresh() {
    ...
    lodTimer = setTimeout(() => {
      lodTimer = null;
      // Never run detail fetches while a drag is moving the view constantly.
      if (drag) return;
      ensureDetail("from");
      ensureDetail("to");
    }, LOD_DEBOUNCE_MS);
  ...
  function drawLane(id: LaneId) {
    ...
    // ── Waveform ───────────────────────────────────────────────────────────
    const peakBuf = pickPeakBuf(lane, viewStart, viewEnd, cssW);
    if (!peakBuf || peakBuf.peaks.length === 0 || lane.durationSecs <= 0) return;
    ...
    for (let col = 0; col < cols; col++) {
      ...
      for (let s = 0; s < steps; s++) {
        ...
        amp = Math.max(amp, peakAt(peakBuf, t));
      }
  ...
  function handleKeydown(e: KeyboardEvent) {
    ...
    // +/- zoom around viewport center — both lanes stay aligned.
    if (e.key === "=" || e.key === "+") {
      e.preventDefault();
      zoomShared(1.15);
...
<div class="mix-win">
  ...
  <div class="mix-body">
    <section class="mix-track-panel" aria-label="Previous track">
      ...
      <div
        class="mix-wave-wrap"
        class:is-dragging={drag?.lane === "from"}
    ...
    <section class="mix-track-panel" aria-label="Next track">
      ...
      <div
        class="mix-wave-wrap"
        class:is-dragging={drag?.lane === "to"}
    ...
    <p class="mix-help">
      <kbd>Ctrl</kbd> + scroll — zoom · scroll — move both · LMB drag — move
      one track · snap: stick under cursor → other stick ·
      <kbd>Alt</kbd> drag — no snap · <kbd>+</kbd>/<kbd>-</kbd> zoom ·
